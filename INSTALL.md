...
1. Visit the [Releases page](https://github.com/volvlabs/towncryer-chat-server/releases/), choose the latest or otherwise the most suitable release. From the list of binaries download the one for your database and platform. Once the binary is downloaded, unpack it to a directory of your choosing, `cd` to that directory.
...
4. Fetch, build Tinode server and tinode-db database initializer:
  - **MySQL**:
	```
	go install -tags mysql github.com/volvlabs/towncryer-chat-server/server@latest
	go install -tags mysql github.com/volvlabs/towncryer-chat-server/tinode-db@latest
	```
  - **PostgreSQL**:
	```
	go install -tags postgres github.com/volvlabs/towncryer-chat-server/server@latest
	go install -tags postgres github.com/volvlabs/towncryer-chat-server/tinode-db@latest
	```
  - **MongoDB**:
	```
	go install -tags mongodb github.com/volvlabs/towncryer-chat-server/server@latest
	go install -tags mongodb github.com/volvlabs/towncryer-chat-server/tinode-db@latest
	```
  - **RethinkDb**:
	```
	go install -tags rethinkdb github.com/volvlabs/towncryer-chat-server/server@latest
	go install -tags rethinkdb github.com/volvlabs/towncryer-chat-server/tinode-db@latest
	```
  - **All** (bundle all of the above DB adapters):
	```
	go install -tags "mysql rethinkdb mongodb postgres" github.com/volvlabs/towncryer-chat-server/server@latest
	go install -tags "mysql rethinkdb mongodb postgres" github.com/volvlabs/towncryer-chat-server/tinode-db@latest
	```

    The steps above install Tinode binaries at `$GOPATH/bin/`, sorces and supporting files are located at `$GOPATH/pkg/mod/github.com/volvlabs/towncryer-chat-server@vX.XX.X/` where `X.XX.X` is the version you installed, such as `0.19.1`.
    ...
    ```
    go install -tags mysql -ldflags "-X main.buildstamp=`date -u '+%Y%m%dT%H:%M:%SZ'`" github.com/volvlabs/towncryer-chat-server/server@latest
    ```
    The value of `buildstamp` will be sent by the server to the clients.

    Building with Go 1.17 or below **will fail**!

5. Open `tinode.conf` (located at `$GOPATH/pkg/mod/github.com/volvlabs/towncryer-chat-server@vX.XX.X/server/`). Check that the database connection parameters are correct for your database. If you are using MySQL make sure [DSN](https://github.com/go-sql-driver/mysql#dsn-data-source-name) in `"mysql"` section is appropriate for your MySQL installation. Option `parseTime=true` is required.
```js
	"mysql": {
...
If you followed instructions in the previous section then the Tinode binaries are installed in `$GOPATH/bin/`, the sources and supporting files are located in `$GOPATH/pkg/mod/github.com/volvlabs/towncryer-chat-server@vX.XX.X/`, where `X.XX.X` is the version you installed, for example `0.19.1`.

Switch to sources directory (replace `X.XX.X` with your actual version, such as `0.19.1`):
```
cd $GOPATH/pkg/mod/github.com/volvlabs/towncryer-chat-server@vX.XX.X
```
...
For more details see https://github.com/volvlabs/towncryer-chat-server/issues/25.
